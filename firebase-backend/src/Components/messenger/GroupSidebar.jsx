...
export const GroupSidebar = ( { id, name, groupPictureUrl } ) => {
    ...
    const handleSave = () => {
        ...
        localStorage.setItem("groupPictureUrl", groupPictureUrl);
    };

    useEffect( () => db.collection( "groups" ).doc( id ).collection( "messages" ).orderBy( "timestamp", "desc" ).onSnapshot( ( snapshot ) => setMessages( snapshot.docs.map( ( doc ) => doc.data() ) ) ), [ id ] );

    return (
        <NavLink className="flex p-2 dark:hover:bg-opacity-10 hover:bg-gray-100 items-center hover:no-underline hover:text-black transition duration-300 animate__animated animate__fadeIn animate__faster cursor-pointer rounded-lg" activeClassName="bg-blue-50 dark:bg-opacity-10" to={ { pathname:`/messenger/group/:${ id }`, props:{ id, name, groupPictureUrl } } } onClick={ handleSave }>
            <img className="h-14 w-14 rounded-full object-cover" src={ groupPictureUrl } alt="" />
            <div className="ml-3 relative w-full">
                <h2 className="dark:text-ghost-white text-lg font-medium -mb-1">{ name.slice( 0, 15 ) }</h2>
                { messages[0]?.message && <p className="dark:text-gray-300 text-gray-700 text-sm">{ messages[0]?.name === user.displayName ? ( <p className="inline-flex">You</p> ) : ( messages[0]?.name.split(' ')[0] ) }<p className="mr-px inline-flex">:</p> { messages[0]?.message.slice( 0, 13 ) }<div className="h-0.5 w-0.5 bg-gray-600 absolute ml-2 bottom-2 inline-flex rounded-full"></div> { messages[0]?.timestamp && <ReactTimeago className="dark:text-gray-300 text-gray-700 ml-4 animate__animated animate__fadeIn" date={ new Date( messages[0]?.timestamp?.toDate() ).toUTCString() } locale="en-US" timeStyle="twitter" /> } </p> }
                { messages[0]?.gifUrl && <p className="dark:text-gray-300 text-gray-700 text-sm">{ messages[0]?.name === user.displayName ? ( <p className="inline-flex">You</p> ) : ( messages[0]?.name.split(' ')[0] ) }<p className="mr-1 inline-flex">:</p><p className="dark:text-gray-300 font-semibold inline-flex">GIF</p><div className="dark:bg-gray-300 h-0.5 w-0.5 bg-gray-600 absolute ml-1.5 bottom-2 inline-flex rounded-full"></div> { messages[0]?.timestamp && <ReactTimeago className="dark:text-gray-300 text-gray-700 ml-3 animate__animated animate__fadeIn" date={ new Date( messages[0]?.timestamp?.toDate() ).toUTCString() } locale="en-US" timeStyle="twitter" /> } </p> }
                { messages[0]?.imageUrl && <p className="dark:text-gray-300 text-gray-700 text-sm">{ messages[0]?.name === user.displayName ? ( <p className="inline-flex">You</p> ) : ( messages[0]?.name.split(' ')[0] ) }<p className="mr-1 inline-flex">:</p><p className="dark:text-gray-300 font-semibold inline-flex">Photo</p><div className="dark:bg-gray-300 h-0.5 w-0.5 bg-gray-600 absolute ml-1.5 bottom-2 inline-flex rounded-full"></div> { messages[0]?.timestamp && <ReactTimeago className="dark:text-gray-300 text-gray-700 ml-3 animate__animated animate__fadeIn" date={ new Date( messages[0]?.timestamp?.toDate() ).toUTCString() } locale="en-US" timeStyle="twitter" /> } </p> }
                { messages[0]?.emojiUrl && <p className="dark:text-gray-300 text-gray-700 text-sm">{ messages[0]?.name === user.displayName ? ( <p className="inline-flex">You</p> ) : ( messages[0]?.name.split(' ')[0] ) }<p className="mr-1 inline-flex">:</p><p className="dark:text-gray-300 font-semibold inline-flex">EMOJI</p><div className="dark:bg-gray-300 h-0.5 w-0.5 bg-gray-600 absolute ml-1.5 bottom-2 inline-flex rounded-full"></div> { messages[0]?.timestamp && <ReactTimeago className="dark:text-gray-300 text-gray-700 ml-3 animate__animated animate__fadeIn" date={ new Date( messages[0]?.timestamp?.toDate() ).toUTCString() } locale="en-US" timeStyle="twitter" /> } </p> }
                { messages[0]?.stickerUrl && <p className="dark:text-gray-300 text-gray-700 text-sm">{ messages[0]?.name === user.displayName ? ( <p className="inline-flex">You</p> ) : ( messages[0]?.name.split(' ')[0] ) }<p className="mr-1 inline-flex">:</p><p className="dark:text-gray-300 font-semibold inline-flex">Sticker</p><div className="dark:bg-gray-300 h-0.5 w-0.5 bg-gray-600 absolute ml-1.5 bottom-2 inline-flex rounded-full"></div> { messages[0]?.timestamp && <ReactTimeago className="dark:text-gray-300 text-gray-700 ml-3 animate__animated animate__fadeIn" date={ new Date( messages[0]?.timestamp?.toDate() ).toUTCString() } locale="en-US" timeStyle="twitter" /> } </p> }
                { messages[0]?.thumbUp && 
                ...
                <p className="absolute bottom-px right-1 font-semibold text-xs text-blue-500">Group</p>
            </div>
        </NavLink>
    );
};